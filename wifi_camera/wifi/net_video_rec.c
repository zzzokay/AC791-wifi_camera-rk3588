...
static void rt_stream_cmd_analysis(u8 chl, u32 add)
{
    ...
    __this_net->net_videoreq[chl].rec.width =
        info->width ? info->width : CONFIG_VIDEO_IMAGE_W;

    __this_net->net_videoreq[chl].rec.height =
        info->height ? info->height : CONFIG_VIDEO_IMAGE_H;

    // 0 = JPEG(AVI), 1 = H264(MOV)
    if (info->type == NET_VIDEO_FMT_MOV) {
        __this_net->net_videoreq[chl].rec.format = 1;
    }
    else {
        __this_net->net_videoreq[chl].rec.format = 0;
    }

    __this_net->net_videoreq[chl].rec.fps =
        info->fps ? info->fps : net_video_rec_get_fps();

    __this_net->net_videoreq[chl].rec.real_fps =
        info->fps ? info->fps : net_video_rec_get_fps();

    __this_net->priv = info->priv;

    printf(">>>>>>>> ch=%d type=%d w=%d h=%d fps=%d format=%d\n",
        chl,
        info->type,
        __this_net->net_videoreq[chl].rec.width,
        __this_net->net_videoreq[chl].rec.height,
        __this_net->net_videoreq[chl].rec.real_fps,
        __this_net->net_videoreq[chl].rec.format);
}
...
static void net_rec_dev_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case VIDEO_SERVER_PKG_NET_ERR:
        ...
        if (__this_net->net_video0_vrt_on) {
            ...
            //__this_net->is_open = FALSE;
            printf("VIDEO_SERVER_PKG_NET_ERR !!! fcnt=%d ffil=%d state=%d vrt=%d art=%d\n",
                __this_net->fbuf_fcnt,
                __this_net->fbuf_ffil,
                __this_net->net_state,
                __this_net->net_video0_vrt_on,
                __this_net->net_video0_art_on); 
