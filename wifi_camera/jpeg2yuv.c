...
#define JPEG_DEC_MCU_YUV420	0 //1:分行解码统一输出YUV420(省内存), 0:输出原始YUV格式

#define JPEG_FPS_DBG		0 //1:打开jpeg解码帧率测试
#define JPEG_RECV_USE_LBUF	0 //1:jpg接收使用lbuf
...
int get_jpeg2yuv_yuv_type(void)
{
    return __this->data_type;
}
...
yuv_malloc:
                ...
                __this->yuv420_1 = malloc(len);
                __this->data_type = ytype;
                if (!__this->yuv420 || !__this->yuv420_1) {
                ...
                free(__this->yuv420);
                free(__this->yuv420_1);
                goto yuv_malloc;
            }

            extern u8 get_lcd_show_deta_mode(void);
            if (get_lcd_show_deta_mode() != UI) {
                extern void get_data_use_flag(void);
                extern void get_data_use_flag1(void);
                if (time == 0) {
                    time = 1;
                    if (time1 == 1) {
                        get_data_use_flag();//等上一包数据发送完成
                    } else {
                        time1++;
                    }
                    yuv = __this->yuv420;
...
#if JPEG_DEC_USE_MUC
    ret = thread_fork("jpeg_frame_mcu_decoder", 9, 1024 * 1, 0, 0, jpeg_frame_mcu_decoder, NULL);
#else
    ret = thread_fork("jpeg_frame_decoder", 9, 1024 * 1, 0, 0, jpeg_frame_decoder, NULL);
#endif
    if (ret) {
        __this->init = 0;
        if (__this->jpeg_buf) {
...
void jpeg2yuv_close(void)
{
    if (__this->init) {
        ...
        extern void clear_os_pend_cnt(void);
        clear_os_pend_cnt();
    }
}
